...
export default function index({ member, token }) {
  ...
  const [content, setContent] = useState(member.image);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(`${API_URL}/api/teams/${member.slug}`, {
      method: 'POST',
      headers: {
    ...
    if (res.ok) {
      toast.success('Saved: Position edited successfully');
      setTimeout(() => {
        setContent(null);
      ...
      <div className="w-1/2">
        ...
        <header className="flex flex-col ">
          <div className="flex items-center mb-[1.6rem]">
            <h3 className="text-black/90 mr-[1.6rem] capitalize">{member.name}</h3>
        ...
        <form action="" className="mt-[4rem]">
          ...
          <Input
            ...
            required={'required'}
            after={`${content.substring(0, 40)}...` || 'Upload an image'}
            className={'mb-[2.4rem] '}
            classInput={
...
export async function getServerSideProps({ req, query: { member } }) {
  const { token } = parseCookies(req);
  const res = await fetch(`${API_URL}/api/teams/${member}`, {
    method: 'GET',
    headers: {
